    ...
    stages {
        ...
        stage('Run ZAP container') {
            steps {
                script {
                    sh '''
                    docker run --rm -v /var/jenkins_home/workspace/$(JOB_NAME)/:/zap/wrk/:rw -u zap -i ghcr.io/zaproxy/zaproxy:stable zap.sh -addoninstall communityScripts -addoninstall jython -loglevel debug -cmd -autorun /zap/wrk/config-mfa.yaml
                    '''
                }
            }
        }
